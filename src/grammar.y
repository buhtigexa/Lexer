        ...
        Temas:
              ...
              E3        :       Conversiones Explicitas,Tercetos.
                                ...
                                uint long   |    uint a long  |   long a uint
                                            |    tolong()     |   touint()
                                -------------------------------------------
               E4       :       Errores en tiempo de ejecucion c) División por 0.
...
programa:
        ...
        | '{'declaraciones'}'
                        //{syntaxError("Se esperan sentencias ejecutables a continuación de las declaraciones.");}
...
declaracion: d_variable  //{ informarSentencia("Declaración de identificadores");}
           ;
...
conjunto_sentencias : sentencia
                    ...
                    |  /* { syntaxError("Se esperan sentencias ejecutables antes de la declaración de un ámbito.");
                          decorator->nroAnidamiento = decorator->nroAnidamiento + 1;
                                  if ( decorator->nroAnidamiento == 1 ){
                                                                   decorator->nroAmbito  = aux;
                                                                   decorator->nroAmbito ++;
...
ambito  :   /*{ informarSentencia("Declaración de ambito.");}*/  declaraciones conjunto_sentencias

        |  /*{ informarSentencia("Declaración de ambito.");}*/ conjunto_sentencias

        |   error   /*{ syntaxError("Declarción de ambito erroneo");}*/
        ;
...
asignacion      :        lado_izquierdo T_ASSIGN exp_ar        /* {        codeGenerator->generarTerceto(":=",false);
                ...
                |        lado_izquierdo T_EQ error              /*{ syntaxError("Asignacion:  Operador y expresiones de asignacion erróneas."); }*/
                |        lado_izquierdo error                   /*{ syntaxError("Asignacion:  expresión no válida.");}*/
                ;
...
factor          :        variable
                ...
                | '('exp_ar')'                                /* {
                                                                  syntaxError("No se permiten expresiones aritméticas entre paréntesis.");
                                                               }*/
...
variable        :       T_IDENTIFIER                /*{      Fila *refDecl = verifyDeclaration (decorator,($1.ptr));
                ...
                |       T_IDENTIFIER '++'           // THIS IS THE NEW STUFF
...
GeneradorTercetos *codeGenerator;

string token;
string stringEmpty;

void initParser(ALexico *a, TablaDeSimbolos *t,GeneradorTercetos *cG, nameDecorator *nm) {
...
static Hashtable<String, Short> codes;
  ...
  codes = new HastTable<String,Short>();
...
int yylex()
{
    ...
    Object row=lexer.getToken();
	if (row.containsKey(row.getToken())){
		code=row.getKey(row.getToken());
...
El conocimiento del tipo de una variable se realiza en la etapa de generacion de codigo intermedio,es importante
observar que las variables ya se encuentran en la tabla de simbolos desde el proceso de tokenizacion.

*******************************************************************************************************************************/
/*
void updateDeclaration ( Fila *symbolTableRow, string identifierType) {
        if ( ( symbolTableRow != NULL ) && ( !symbolTableRow->isEmptyType() ) )
                TForm1::writer("Línea " + Lexer->getNroLinea() + ":  El identificador  " +   symbolTableRow->getLexeme()+ "  ya ha sido declarado.");
        else
